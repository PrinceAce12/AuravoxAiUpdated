...
CREATE OR REPLACE FUNCTION public.set_webhook_settings_created_by()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS on_webhook_settings_created ON public.webhook_settings;
CREATE TRIGGER on_webhook_settings_created
    BEFORE INSERT ON public.webhook_settings
    FOR EACH ROW EXECUTE FUNCTION public.set_webhook_settings_created_by();
...
-- Insert a default webhook setting (optional - can be configured later)
